// majay/app/api/maintenance/route.ts
import { google } from "googleapis"
import { JWT } from "google-auth-library"

export async function POST(request: Request) {
  try {
    const data = await request.json()
    const credentials = JSON.parse(process.env.GOOGLE_SHEETS_CREDENTIALS!)
    const auth = new JWT({
      email: credentials.client_email,
      key: credentials.private_key,
      scopes: ["https://www.googleapis.com/auth/spreadsheets"],
    })
    const sheets = google.sheets({ version: "v4", auth })

    const dateValue = new Date(data.date).toLocaleDateString('en-US', {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit'
    })
    const id = Date.now().toString()

    const row = [
      id,
      dateValue,
      data.description,
      Number(data.cost),
      data.status
    ]

    await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.NEXT_PUBLIC_GOOGLE_SHEET_ID,
      range: "Maintenance Log!A:E",
      valueInputOption: "USER_ENTERED",
      insertDataOption: "INSERT_ROWS",
      requestBody: { values: [row] }
    })

    return Response.json({ 
      success: true, 
      data: { 
        id,
        ...data
      }
    })
  } catch (error) {
    console.error("Maintenance API error:", error)
    return Response.json({ error: "Failed to record maintenance" }, { status: 500 })
  }
}

export async function GET() {
  try {
    const credentials = JSON.parse(process.env.GOOGLE_SHEETS_CREDENTIALS!)
    const auth = new JWT({
      email: credentials.client_email,
      key: credentials.private_key,
      scopes: ["https://www.googleapis.com/auth/spreadsheets"],
    })
    const sheets = google.sheets({ version: "v4", auth })

    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.NEXT_PUBLIC_GOOGLE_SHEET_ID,
      range: "Maintenance Log!A:E"
    })

    const rows = response.data.values || []
    const maintenanceRecords = rows.slice(1).map((row) => ({
      id: row[0],
      date: row[1],
      description: row[2],
      cost: Number(row[3]),
      status: row[4] || "Pending"
    }))

    return Response.json(maintenanceRecords)
  } catch (error) {
    console.error("Failed to fetch maintenance records:", error)
    return Response.json({ error: "Failed to fetch maintenance records" }, { status: 500 })
  }
}
